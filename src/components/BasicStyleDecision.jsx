...
export function BasicStyleDecision({ elements, matrixState, requirements, historicalCombos, basicDecisions, setBasicDecisions, onSubmit }) {
  ...
  const analyzedCombos = useMemo(() => {
    ...
    return historicalCombos.map(combo => {
      ...
      const estUnitCost = Math.round((fixedCost / sales) + variableCost);
          ...
          <div className="flex justify-between items-center bg-[#f9fafb] p-3 border border-[#d1d5db] rounded-t-lg mb-2 shrink-0">
             ...
             <div className="flex items-center gap-4">
                ...
                <button 
                  ...
                  className={`px-2 py-1 text-[10px] rounded border ${sortBy === 'trend' ? 'bg-primary-50 text-primary border-primary font-bold' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'}`}
                >流行度</button>
                <button 
                ...
                <button 
                  ...
                  className={`px-2 py-1 text-[10px] rounded border ${sortBy === 'fit' ? 'bg-status-good-bg text-status-good-text border-status-good-border font-bold' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'}`}
                >契合度</button>
             </div>
             </div>
          </div>
            ...
            return leftList.map(combo => {
              ...
              return (
                <div key={combo.id} className={`border rounded-lg p-4 bg-white shadow-sm flex flex-col gap-4 ${
                  currentDecision === 'approve' ? 'border-status-good-border ring-1 ring-status-good-border' :
                  currentDecision === 'reject' ? 'border-status-bad-border opacity-75 ring-1 ring-status-bad-border' : currentDecision === 'fine_tune' ? 'border-status-warn-border ring-1 ring-status-warn-border' : 'border-[#d1d5db]'
                }`}>
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="text-lg font-bold text-content-main">{combo.name}</h3>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {combo.elements.map((el, idx) => {
                          ...
                          return (
                            <span key={`${el.id}-${idx}`} className={`text-[10px] px-1.5 py-0.5 rounded border ${isMissing ? 'bg-status-bad-bg text-status-bad-text border-status-bad-border line-through' : 'bg-white text-[#374151] border-[#d1d5db]'}`}>
                              {elName}
                            </span>
                          );
                        })}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 h-8">
                        <div className="flex flex-col">
                          ...
                          <span className={`text-sm font-bold ${combo.hasEnoughMatches ? 'text-status-good-text' : 'text-status-bad-text'}`}>
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className={`text-sm font-bold ${combo.totalFitScore >= 10 ? 'text-status-good-text' : combo.totalFitScore > 0 ? 'text-status-warn-text' : 'text-status-bad-text'}`}>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 h-8">
                        <div className="flex flex-col">
                          ...
                          <span className="text-sm font-bold text-primary">{(combo.estMargin * 100).toFixed(0)}%</span>
                        </div>
                        <div className="w-px h-6 bg-[#d1d5db]"></div>
                        <div className="flex flex-col">
                ...
                <div className="p-4 border-b border-[#e5e7eb] bg-[#f9fafb] rounded-t-lg shrink-0">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-[#6b7280] font-bold bg-surface-base px-2 py-1 rounded-full">已選 {devList.length} 款</span>
                    </div>
                    <div className="flex gap-2">
                      <span className="text-xs text-gray-500 font-bold self-center mr-1">排序:</span>
                      <button 
                        onClick={() => setRightSortBy('trend')} 
                        className={`px-2 py-1 text-[10px] rounded border ${rightSortBy === 'trend' ? 'bg-primary-50 text-primary border-primary font-bold' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'}`}
                      >流行度</button>
                      <button
                      ...
                      <button 
                        ...
                        className={`px-2 py-1 text-[10px] rounded border ${rightSortBy === 'fit' ? 'bg-status-good-bg text-status-good-text border-status-good-border font-bold' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'}`}
                      >契合度</button>
                      <button 
                        ...
                        {devList.map(combo => (
                          <tr key={`dev-${combo.id}`} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-2 py-3 text-center">
                              <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${basicDecisions[combo.id] === 'approve' ? 'bg-status-good-bg text-status-good-text' : 'bg-status-warn-bg text-status-warn-text'}`}>
                            ...
                            <td className={`px-2 py-3 text-right font-bold ${combo.competitionScore > 1.0 ? 'text-status-bad-text' : combo.competitionScore > 0.5 ? 'text-status-warn-text' : 'text-status-good-text'}`}>
