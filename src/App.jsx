...
import { Step2SetupEvent, Step3Dashboard, Step4Designer, Step5QA } from './components/MockPages';
...
function Dashboard({ onStepChange }) {
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <h1 className="text-2xl font-bold text-[#111827] border-b border-[#d1d5db] pb-4">商品開發中心</h1>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {/* 開發決策輔助 */}
        <div className="bg-white border border-[#2563eb] rounded-lg shadow-sm overflow-hidden flex flex-col">
          <div className="bg-[#2563eb] text-white p-4 font-bold text-lg">開發決策輔助</div>
          <div className="p-4 flex-1 flex flex-col gap-3">
            <button onClick={() => onStepChange(1)} className="w-full text-left p-3 border border-[#d1d5db] rounded hover:border-[#2563eb] hover:bg-[#eff6ff] transition-colors flex items-center justify-between group">
              <span className="font-medium text-[#374151] group-hover:text-[#2563eb]">初步決定開發方向</span>
              <span className="text-[#2563eb] opacity-0 group-hover:opacity-100">&rarr;</span>
            </button>
            <button onClick={() => onStepChange(2)} className="w-full text-left p-3 border border-[#d1d5db] rounded hover:border-[#2563eb] hover:bg-[#eff6ff] transition-colors flex items-center justify-between group">
              <span className="font-medium text-[#374151] group-hover:text-[#2563eb]">建立群眾預測活動</span>
              <span className="text-[#2563eb] opacity-0 group-hover:opacity-100">&rarr;</span>
            </button>
            <button onClick={() => onStepChange(3)} className="w-full text-left p-3 border border-[#d1d5db] rounded hover:border-[#2563eb] hover:bg-[#eff6ff] transition-colors flex items-center justify-between group">
              <span className="font-medium text-[#374151] group-hover:text-[#2563eb]">最終決定開發計畫</span>
              <span className="text-[#2563eb] opacity-0 group-hover:opacity-100">&rarr;</span>
            </button>
          </div>
        </div>
...
function App() {
          ...
          <div className="space-y-3">
            {['商品企劃', '趨勢分析師', '設計師', '數據分析師', '採購', '財務'].map(role => (
              <button
                ...
                className="w-full text-left px-4 py-3 border border-[#d1d5db] rounded hover:border-[#2563eb] hover:bg-[#eff6ff] transition-colors text-[#374151] font-medium flex justify-between items-center group"
              >
                <span>{role}</span>
                <span className="text-[#2563eb] opacity-0 group-hover:opacity-100 text-sm">登入 &rarr;</span>
              </button>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-[#e5e7eb] font-sans text-[#111827] overflow-hidden">
      {/* Header */}
      ...
      <div className="h-10 bg-white border-b border-[#d1d5db] flex items-center px-4 text-sm text-[#4b5563] shrink-0">
        <button onClick={() => setCurrentStep(0)} className="hover:text-[#2563eb] hover:underline">商品開發</button>
        <span className="mx-2 text-[#9ca3af]">&gt;</span>
        {currentStep === 0 ? (
          <span className="font-semibold text-[#111827]">總覽</span>
        ) : (
          <>
            <button onClick={() => setCurrentStep(0)} className="hover:text-[#2563eb] hover:underline">開發決策輔助</button>
            <span className="mx-2 text-[#9ca3af]">&gt;</span>
